...
app.get('/sfu/*', (req, res, next) => {
    const path = '/sfu/'
    // Before proceeding, ensure the last one has joined the room?
    ...
    if (req.path.indexOf(path) == 0 && req.path.length > path.length) {
        ...
        // eventEmitter.emit('enqueue'); // Trigger queue processing
        // processQueue()
        // return
    } else {
...
//         next()
//     }
//     // isProcessing = false; // ?If this line isn't here the code doesn't work
// }
...
// let secondarySFUActive = false // not required for the time being
